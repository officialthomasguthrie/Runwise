...
export const ReactFlowEditor = ({
  ...
  // Auto-save when a webhook path is set so the URL is immediately active
  useEffect(() => {
    if (webhookAutoSavePendingRef.current) {
      webhookAutoSavePendingRef.current = false;
      handleSave().catch(() => {/* silently ignore auto-save errors */});
    }
  ...
  const handleNodeConfigUpdate = useCallback((nodeId: string, config: Record<string, any>) => {
    setNodes((prevNodes) => {
      const updatedNodes = prevNodes.map((node) => {
        if (node.id === nodeId) {
          // If this is a webhook-trigger node and the path just became non-empty,
          // flag an auto-save so the webhook URL is immediately active in the DB.
          const nodeData = node.data as any;
          if (nodeData?.nodeId === 'webhook-trigger' && config.path && String(config.path).trim().length > 0) {
            webhookAutoSavePendingRef.current = true;
          }
          return {
            ...node,
            data: {
  ...
  return (
    <div style={{ width: '100%', height: '100%' }} className="bg-background relative">
      <ReactFlow
        ...
        minZoom={0.1}
        maxZoom={4}
        className="runwise-workflow-builder bg-background"
