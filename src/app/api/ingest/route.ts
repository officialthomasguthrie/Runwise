...
// Note: This route should match /api/inngest for consistency
const allFunctions = [
  helloWorld,
  testWorkflow,
  sendTestNotification,
...
if (typeof window === 'undefined') {
  try {
    ...
    console.log('[Inngest (ingest route)] Registering', allFunctions.length, 'functions:', functionIds.join(', '));
    ...
    if (undefinedFunctions.length > 0) {
      console.error('[Inngest (ingest route)] ERROR: Some functions are undefined:', undefinedFunctions);
    }
    ...
    if (!hasScheduled) {
      console.error('[Inngest (ingest route)] CRITICAL: scheduled-workflow-trigger function is missing!');
    }
    // Polling triggers are now handled by Cloudflare Worker (cron every 1 minute)
  } catch (error) {
    console.error('[Inngest (ingest route)] Error logging function info:', error);
  }
}
...
try {
  ...
  console.log('[Inngest (ingest route)] Serve handler created successfully');
} catch (error: any) {
  console.error('[Inngest (ingest route)] CRITICAL: Failed to create serve handler:', error);
  throw error;
}
