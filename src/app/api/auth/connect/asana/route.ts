/**
 * Asana OAuth Connect Route
 * Initiates OAuth flow for Asana integration
 */

import { NextRequest, NextResponse } from 'next/server';
import { createClient } from '@/lib/supabase-server';
import { getAsanaAuthUrl, generateStateToken } from '@/lib/integrations/oauth';
...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log('[API /auth/connect/asana] Initiating Asana OAuth flow');
    ...
    const response = NextResponse.redirect(getAsanaAuthUrl(state));
    ...
    console.error('Error initiating Asana OAuth:', error);
    return NextResponse.json(
