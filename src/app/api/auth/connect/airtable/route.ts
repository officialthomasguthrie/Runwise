/**
 * Airtable OAuth Connect Route
 * Initiates OAuth flow for Airtable integration
 */

import { NextRequest, NextResponse } from 'next/server';
import { createClient } from '@/lib/supabase-server';
import { getAirtableAuthUrl, generateStateToken } from '@/lib/integrations/oauth';
...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log('[API /auth/connect/airtable] Initiating Airtable OAuth flow');
    ...
    const response = NextResponse.redirect(getAirtableAuthUrl(state));
