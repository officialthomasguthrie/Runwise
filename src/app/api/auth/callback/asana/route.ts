 ...
 * Note: Asana tokens can be refreshed
 */

import { NextRequest, NextResponse } from 'next/server';
import { createClient } from '@/lib/supabase-server';
import { storeUserIntegration } from '@/lib/integrations/service';
import { validateStateToken, getAsanaOAuthConfig } from '@/lib/integrations/oauth';
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (error) {
      console.error('[Asana OAuth Callback] OAuth error:', error);
    ...
    if (!code || !state) {
      console.error('[Asana OAuth Callback] Missing code or state:', { 
        code: !!code, 
        state: !!state
      });
      return NextResponse.redirect(
    ...
    console.log('[Asana OAuth Callback] Validating state:', { 
    ...
    if (!storedState || !validateStateToken(state, storedState)) {
      console.error('[Asana OAuth Callback] Invalid state token');
      return NextResponse.redirect(
    ...
    if (!userId) {
      console.error('[Asana OAuth Callback] No user ID in cookie');
      return NextResponse.redirect(
    ...
    if (authError || !user || user.id !== userId) {
      console.error('[Asana OAuth Callback] Auth error or user mismatch:', { 
    ...
    // Asana uses POST to /-/oauth_token
    const tokenUrl = 'https://app.asana.com/-/oauth_token';
    
    console.log('[Asana OAuth Callback] Exchanging code for tokens:', { 
      hasCode: !!code, 
    ...
    if (!tokenResponse.ok) {
      ...
      console.error('[Asana OAuth Callback] Token exchange error:', error);
      return NextResponse.redirect(
    ...
    console.log('[Asana OAuth Callback] Token exchange successful');
    
    // Asana tokens can expire (expires_in is in seconds)
    // If expires_in is not provided, token doesn't expire
    const expiresAt = tokens.expires_in
      ? new Date(Date.now() + tokens.expires_in * 1000)
      : undefined;
    
    // Store integration
    console.log('[Asana OAuth Callback] Storing integration for user:', user.id);
    try {
      await storeUserIntegration(
        ...
        },
        {
          ...
          data: tokens.data // Asana returns additional data in 'data' field
        }
      );
      console.log('[Asana OAuth Callback] Integration stored successfully');
    } catch (storeError: any) {
      console.error('[Asana OAuth Callback] Error storing integration:', storeError);
      return NextResponse.redirect(
    ...
    const successParam = isWorkspace ? 'connected' : 'asana_connected';
    ...
    console.error('[Asana OAuth Callback] Unexpected error:', error);
    const returnUrl = request.cookies.get('oauth_return_url')?.value || '/settings?tab=integrations';
    const separator = returnUrl.includes('?') ? '&' : '?';
